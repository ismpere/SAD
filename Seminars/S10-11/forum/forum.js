...
var dm = require('./dm_remotezmq.js');
var fs = require('fs');
var util = require('util');

/*var servertolisten=process.argv[2];

var log_file = fs.createWriteStream('./debug'+servertolisten+'.log', {flags : 'w'});
...
*/
var servertolisten=process.argv[2];
console.log("Forum will listen in "+servertolisten);
var servertorequest=process.argv[3];
console.log("Forum will rquest to "+servertolisten);
var servertosubscriber=process.argv[4];
console.log("Forum will subscribe to "+servertosubscriber);

dm.Start(servertorequest);
...
http.listen (servertolisten, on_startup);
...
subscriber.subscribe("webserver");
subscriber.on('message', function(data,mensajes) {
		mensajes=mensajes.toString();
		console.log("recibe mensaje de subscriber");
		console.debug(mensajes);
		io.emit('message', mensajes);
  });

subscriber.connect(servertosubscriber);
