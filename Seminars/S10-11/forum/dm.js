...
exports.addPrivateMessage = function (msg){
	return new Promise((resolve,reject)=>{
		...
		if (!ml){
			privateMessages [k] = [];
			blockchain.CreateRoom(k,{date:Date.now()})
			.then(result=>{
				
				privateMessages[k].push (msg);
				blockchain.PublishMessage(k,privateMessages [k].length,JSON.stringify(msg))
				.then(()=>{
					resolve(msg);
				})
				.catch(error=>{
...
exports.addPublicMessage = function (msg)
{
	...
	console.log("-->>"+JSON.stringify (publicMessages[msg.to]));
}

exports.getPublicMessageList = function (sbj) {
	console.log("..."+sbj+"----"+JSON.stringify (publicMessages[sbj]));
