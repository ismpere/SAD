...
var dmclient = require("./dm-client.js");
...
io.on("connection", function(sock) {
  ...
  sock.on("get user list", function() {
    ...
    // Asynchronous way
    dmclient.getUserList(function(users) {
      sock.emit("user list", users);
    });
    // Synchronous way
    // console.log('Usuarios sincrono: ' + dm.getUserList())
  ...
  sock.on("get subject list", function() {
    ...
    // Asynchronous way
    dmclient.getSubjectList(function(subjects) {
      console.log("Temas: " + subjects);
  ...
  sock.on("get message list", function(from, to, isPriv) {
    ...
    if (isPriv) {
      ...
      // Asynchronous way
      dmclient.getPublicMessageList(to, function(messageList) {
        sock.emit("message list", from, to, isPriv, messageList);
      });
  ...
  sock.on("login", function(u, p) {
    ...
    // Asynchronous way
    dmclient.login(u, p, function(loginSuccess) {
      if (!loginSuccess) {
